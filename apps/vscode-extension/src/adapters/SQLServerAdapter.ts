...
export class SQLServerAdapter extends EventEmitter implements DatabaseAdapter {
  ...
  async getTableMetadata(schema: string, table: string): Promise<ColumnMetadata[]> {
    ...
    return result.recordset.map((row) => ({
      ...
      isForeignKey: false, // Would require additional query
      foreignKeyRef: null,
  ...
  async explainQuery(querySQL: string): Promise<ExplainPlan> {
    // SQL Server uses SET SHOWPLAN_XML or SET STATISTICS XML for execution plans
    ...
    // Parse the XML and convert to ExplainPlan format
    // This is a simplified version - actual implementation would parse XML properly
    return {
      Plan: {
        'Node Type': 'SQL Server Query Plan',
        'Startup Cost': 0,
        'Total Cost': 0,
        'Plan Rows': 0,
        'Plan Width': 0,
    ...
    for (const filter of filters) {
      ...
      switch (filter.operator) {
        ...
        case 'in':
          ...
          if (values.length > 0) {
            const placeholders = values.map((_: unknown, i: number) => `@${paramName}_${i}`).join(', ');
